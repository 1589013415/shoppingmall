...
export class CommodityManage extends Component {
  ...
  render() {
    ...
    let columns = [
      ...
      {
        title: '图片', dataIndex: 'imagePath', key: 'imagePath',
        render: (text) => <Space>
          {
            text.map((e, indexOf) => {
              return <Image
      ...
      {
        title: '操作', dataIndex: 'commodityButton', key: 'commodityButton', align: 'center',
        render: (text, record) =>
          <Space>
            {record.state === 0 ?//是否审核中
              ...
              record.state === 1 || record.state === 2 ? <Space>
                <Button onClick={() => { this.audit(record, "cancel") }} type="primary" danger>
                  取消审核</Button>
                <Button danger>删除商品</Button></Space> :
                record.state === 3 ? <Space>
                  <Button disabled danger>删除商品</Button></Space> : null
